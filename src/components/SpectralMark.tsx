  ...
  if (variant === "gridline") {
    return (
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
        role="img"
        aria-label={alt || "Gridline mark: AI code editor grid"}
        className={className}
      >
        {Array.from({ length: 5 }).map((_, r) =>
          Array.from({ length: 5 }).map((_, c) => (
            <rect
              ...
              stroke="currentColor"
              strokeWidth="1.5"
              strokeOpacity={c === 2 && r === 1 ? "0.8" : "0.2"}
              fill={c === 2 && r === 1 ? "var(--spectral-b)" : "none"}
              fillOpacity={c === 2 && r === 1 ? "0.15" : "0"}
            />
          )),
        )}
        <path
          d="M44 28 L56 28 M44 36 L62 36 M44 44 L52 44"
          stroke="var(--spectral-b)"
          strokeWidth="2"
          strokeLinecap="round"
          opacity="0.6"
        />
        <circle cx="74" cy="30" r="3" fill="var(--spectral-b)" opacity="0.8" />
      </svg>
    );
  }
